...
shinyServer(function(input, output) {
  ...
  output$redPlot <- renderPlot({
    ...
    qplot(number_of_red_balls[1:t], fill="##FF666", binwidth=0.12)  + xlab("X (Number of red balls)") +
      ylab("Frequency") +
  ...
  output$probPlot <- renderPlot({
    ...
    # ggplot(red, aes(x=red)) + geom_density(alpha=.2, fill="#FF6666") +
    #   xlab("X (Number of red balls)") +
    #   ylab("Probability") + theme_few() + coord_cartesian(ylim = c(0, 1))
    r = red$red
    ...
    s = data.frame(s)
    s$x1 = c(0, 1, 2, 3, 4)
    ggplot(s, aes(x=factor(x1), y=s, width=0.25)) + geom_bar(stat="identity", fill="#f98866") +
      theme_minimal() +
      coord_cartesian(ylim = c(0, 1)) + xlab("X (Number of red balls)") +
      ylab("Probability")+theme(legend.position = "none")
